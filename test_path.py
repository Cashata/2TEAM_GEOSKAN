...
DEAD_ZONE_RADIUS = 400 * MAP_SCALE_FACTOR
...
# cv2.imwrite('map.png', map_im * map_mask)
...
print(path)
path_im = cv2.cvtColor(map_im, cv2.COLOR_GRAY2BGR)
for x, y in path:
    cv2.circle(path_im, (int(x), int(y)), 2, (0, 255, 0), cv2.FILLED)
cv2.imshow('path', path_im)
cv2.imwrite('path.png', path_im)
cv2.waitKey(0)
cv2.destroyAllWindows()
